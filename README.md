...
后续反向课程在 `reverse_curriculum.py` 中定义 Stage 1--20 的累计扰动上限：依次打开夹爪、沿抓取轴后退、增加横向和手腕误差，再扩展到手臂/躯干、floating base、物体几何和物体动力学。运行时进度与 reset 难度使用独立标签；形成可靠抓握为进度 Stage 0，其余状态按夹爪开度、轴向距离、横向距离和手腕误差的最坏归一化分量映射到 Stage 1--20。当前提交只定义分区，不启用升级奖励、状态库或 reset 概率调度。
