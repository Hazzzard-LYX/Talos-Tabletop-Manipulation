...
> **2026-09-04 状态说明：初步锚点探索失败。** 800 个候选中只有 66 个通过物理筛选（8.25%），且 20 个 stage 均未达到每级 10 个有效锚点的门槛。当前候选库存在夹爪过闭合、base/下肢无多样性以及方块未与手部目标联合生成等问题，不得用于 2048-env 泛化站立训练。详见[失败报告与下一步计划](docs/initial_anchor_exploration_failure_report_zh.md)。
...
- `Mjlab-Tabletop-Anchor-Standing-Cube-Talos-v0`
...
泛化站立预训练任务 `Mjlab-Tabletop-Anchor-Standing-Cube-Talos-v0` 已实现从 20-stage 锚点库 reset 的代码路径。训练时通过 `TALOS_STANDING_ANCHOR_BANK` 指定 `.pt` 文件，并在锚点附近随机化关节、floating base、物体、脚底摩擦、关节摩擦、PD 增益、编码器偏置和周期速度推扰。domain randomization 共三级；每级按 8192 个完成的 episode 统计连续稳定站立 5 秒的成功率，只有严格高于 80% 才升级。每个 10 秒 episode 的推扰安排在第 6–6.5 秒，使课程先完成连续站立判定、后训练受扰恢复。默认训练规模为 2048 environments，接口仍为 222/225 维观测和 29 维动作，因而可以从同结构站立 checkpoint 初始化。该训练入口目前仅作为基础设施保留，在重新生成并验证合格锚点库前不得用于正式训练。
